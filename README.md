...
따뜻한 위로 글귀와 노랫말을 띄워 온기를 전하며, 우울증 의심 유저의 경우 근처 상담센터의 정보를 추가로 제공하여 전문의와의 상담을 권유합니다.
...
### :musical_note: 적용 기술
<데이터셋>
5가지 감정 - 5가지 감정(기쁨, 중립, 슬픔, 화남, 두려움)으로 레이블링 된 텍스트 데이터셋
<모델>
한국어 자연어 처리 모델 KoBert

3. 공공데이터 분석: 통계청의 지역별, 연령별, 성별, 직업별 자살률 통계 데이터를 참고하여 우울증 취약계층을 파악한 후 이를 우울 지수 계산에 반영한다.
